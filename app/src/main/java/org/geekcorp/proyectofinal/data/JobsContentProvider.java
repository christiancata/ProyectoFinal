...
public class JobsContentProvider extends ContentProvider
{
    ...
    public Uri insert(Uri uri, ContentValues values)
    {
        ...
        {
            case JOBS:
                retValue = dbHelper.getWritableDatabase().
                        insert(JobPostDbContract.JobEntry.TABLE_NAME, null, values);
                if (retValue != -1)
                {
            ...
            case CONTACTS:
                retValue = dbHelper.getWritableDatabase().
                        insert(JobPostDbContract.ContactEntry.TABLE_NAME, null, values);
                if (retValue != -1)
                {
                ...
                {
                    throw new SQLException("Error inserting Contact");
                }
                break;
            default:
                throw new UnsupportedOperationException("Not yet implemented");
